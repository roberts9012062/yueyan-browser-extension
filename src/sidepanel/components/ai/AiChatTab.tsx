...
import { compressScreenshot, cropScreenshot, pickRegion } from './screenshot-tools';
import type { PickRect } from './screenshot-tools';
...
export function AiChatTab(props: AiChatTabProps) {
  ...
  /** 当前面板形态（页面生命周期内不变；embed=悬浮球展开的面板；exec 形态不渲染本组件） */
  const PANEL_MODE: PanelMode = readCurrentMode();
  ...
  /** 按需主机授权（用户手势内申请，仅首次弹框；网页总结与截图共用，实现在 shared/permissions） */
  ...
  async function summarizeBiliVideo(tab: chrome.tabs.Tab): Promise<void> {
    // 字幕元信息抓取复用 shared/bili-video（与右键「总结本页」执行器同一实现）：
    // 注入页面取 cid 与字幕列表（优先 ai-zh AI 中文字幕，回退 CC 中文字幕）
    const meta: BiliSubtitleMeta | undefined = await collectBiliSubtitleMeta(tab.id as number);
    ...
    setSending(true);
    try {
      const transcript: string = await fetchBiliTranscript(meta.subtitleUrl as string);
      if (transcript.trim() === '') {
      ...
      setArticleBiliProps(buildBiliPlayerProps(meta));
  ...
  async function grabPageAndSummarize(): Promise<void> {
    ...
    const biliTab: chrome.tabs.Tab | undefined = candidates.find(
      (tItem: chrome.tabs.Tab): boolean => tItem.id !== undefined && tItem.url !== undefined && isBiliVideoUrl(tItem.url),
    );
          ...
          func: (): { ok: boolean; title: string; url: string; text: string; images: string[] } => {
            ...
            const collectFrom = (scope: ParentNode, picked: string[], seenUrls: Set<string>): void => {
                for (const img of Array.from(scope.querySelectorAll('img'))) {
                    ...
                    // 死图过滤：浏览器已完成加载尝试但无尺寸（404/防盗链等裂图，页面上显示不出来）——
                    // 直接丢弃，不进入总结插图与发布转存链路（裂图转存必失败，0.32.1）
